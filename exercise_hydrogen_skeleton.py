...
radii=np.linspace(0,5,10)
energies,distances=E_v_R(radii)

cs=CubicSpline(distances, energies)

res=minimize(cs,np.min(energies))
E_min=cs(res.x)

plt.plot(np.linspace(0.3,5,100), cs(np.linspace(0.3,5,100)), '--', c='k', alpha=0.5, label='interpolation')
plt.plot(res.x,cs(res.x),'o' , label=r'$R_{eq}$'+f' = {np.round(res.x[0],3)} $a_0$', c='k')
plt.plot(distances,energies,'*', label='calculated')
plt.xlabel(r'R ($a_0$)')
plt.ylabel(r'$E_{tot}$  (Ry)')
plt.ylim(-2,0)
plt.xlim(0,5.1)
plt.legend()
plt.savefig('Question_8a', dpi=300)
plt.show()

R_mask=np.where(energies==np.min(energies))
E_min=np.min(energies)
R_eq=distances[R_mask]
...
def scf_cycle(zeta, dens, totdens,entot,cycles,tol,Plot_conv=True, verbose=True):
    num_c=0
    res=1
    energies=[entot]
    while (num_c<=cycles) & (np.abs(res)>tol):
        num_c+=1
        coulombpot = poisson(totdens, k2) # electronic Coulomb potential
        enx, xpot = hfsa(dens, h) # Hartree-Fock-Slater exchange energy and potential
        pot = coulombpot + xpot # total potential
        ...
        res=entot-energies[-1]
        energies.append(entot)
        
        if verbose:
            print(f'at cycle number {num_c}, energy is {np.round(entot,3)} Ry, convergence is {np.round(res*1/tol,3)}*e{int(np.log10(tol))}')
        
    if Plot_conv:
        plt.plot(np.arange(num_c+1),energies, '--.')
        plt.xlabel('Cycle')
        plt.ylabel('Energy')
        plt.show()
...
if SFC_on:       
    Energies=[]
    Real_dist=[]
    dists=np.linspace(1,5,10)
    for i,d in enumerate(dists):
        dist = d
        shift = int(round(dist / (2 * h)))
        real_dist = 2 * shift * h
        Real_dist.append(real_dist)
        ...
        en=scf_cycle(zeta, dens, totdens,entot,cycles=100,tol=1e-4,Plot_conv=False)
        Energies.append(en)
        
    Real_dist=np.array(Real_dist)
    Energies=np.array(Energies)
    cs_new=CubicSpline(Real_dist, Energies)
    res_new=minimize(cs_new,np.min(Energies))
    E_min_new=cs(res_new.x)
    
    plt.plot(np.linspace(0.3,5,100), cs(np.linspace(0.3,5,100)), '--', c='k', alpha=0.5, label='interpolation no SCF')
    plt.plot(res.x,cs(res.x),'o' , label=r'$R_{eq}$'+f' = {np.round(res.x[0],3)} $a_0$', c='#515152')
    plt.plot(distances, energies,'*', c= 'k', alpha=0.5, label='calculated no SCF')
    plt.plot(np.linspace(0.3,5,100), cs_new(np.linspace(0.3,5,100)), '--', c='r', alpha=0.5, label='interpolation after SCF')
    plt.plot(res_new.x,cs_new(res.x),'o' , label=r'$R_{eq}$'+f' = {np.round(res_new.x[0],3)} $a_0$', c='r')
    plt.plot(Real_dist,Energies,'.', c='r', label='calculated after SCF')
    plt.xlim(0,5)
    plt.ylim(-2,-1)
    plt.legend()
    plt.xlabel(r'R ($a_0$)')
    plt.ylabel(r'$E_{tot}$  (Ry)')
    plt.savefig('Question_8c_SCF_comp', dpi=300)
    plt.show()
    
    E_H=1.33
    min_e=np.where(np.array(Energies)==np.array(Energies).min())
    E_eq=np.array(Energies).min()
    Rn_eq=np.array(Real_dist)[min_e]
    E_b=E_eq-2*-0.734
